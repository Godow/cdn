setTimeout(() => {
  ...
  function readSettings() {
    ...
    settings.fontStyle = {
      error:
        "font-family: 微软雅黑,黑体,Droid Serif,Arial,sans-serif; color: #f00;",
      ...
      maxTip:
        "font-family: 微软雅黑,黑体,Droid Serif,Arial,sans-serif; font-size: 30px; background-color: #222; text-shadow: 0px 0px 12px #fff; color: #fff;",
    };
    ...
    if (window === top) {
       ...
       */
      settings.NoAD解析 = {
        // TODO by 17kyun.com/api.php?url=    // TODO by tv.hzwdd.cn
        ...
        OK解析:
          ...
          "https://okjx.cc/?url=" ||
          "https://m2090.com/?url=", // TODO 优质: 腾讯 (爱奇艺) 优酷 乐视 芒果 PPTV (华数)
        全民解析:
          "https://jx.quanmingjiexi.com/?url=" ||
          "https://chaxun.truechat365.com/?url=", // TODO 已失效
        ...
        虾米解析:
          "https://jx.xmflv.com/?url=" || "https://nbjx.vip/index.php?url=", // TODO (土豆) (咪咕) 搜狐 (芒果)
      ...
      settings.AD解析 = {
        // TODO (有赌博广告，请勿相信，这么简单的骗术不会有人上当吧)
      ...
      top.postMessage(
        key.ENCRYPT(
          "宝塔镇河妖\x00给予\x000\x00" + location.href,
          settings.k1,
          settings.k2
        ),
        "*"
      ...
      settings.parseDB = new Promise((resolve) => {
        window.addEventListener("message", (event) => {
          if (event.source !== window) {
            try {
              const sql = key
                .DECRYPT(event.data, settings.k1, settings.k2)
                .split("\x00");
              if (sql[0] === "天王盖地虎") {
                if (sql[1] === "给予") {
                  if (sql[2] === "用户数据库") {
    ...
    class key {
        ...
        if (pwd) {
          if (typeof pwd === "string") {
            ...
            console.error(
              "Unsupported type '" +
                typeof pwd +
                "'. It only supports 'string' or 'number'."
            );
          }
        }
      ...
      encrypt(string) {
        if (string) {
          ...
          let subStart = string.length,
            subIndex = parseInt(this[password][0]) + 3,
            encryptPool = [],
            result = "";
          // stringKey 加密字符偏移量的散列值的的个数。
          const stringKey = subStart + parseInt(this[password].substring(1));
          // stringKeyFloat 加密字符偏移量的偏移单位。stringKeyIndex 加密字符偏移量的位置
          let stringKeyFloat = 65536 / stringKey,
            stringKeyIndex = stringKey - 1;
          // 获取加密池。
                  ...
                  (char.charCodeAt() +
                    parseInt(
                      stringKeyFloat *
                        (stringKeyIndex % 2 === 0
                          ? stringKeyIndex
                          : stringKey - stringKeyIndex)
                    )) %
                    65536
                );
                stringKeyIndex =
                  stringKeyIndex === 0 ? stringKey - 1 : stringKeyIndex - 1;
              } else {
                break;
              }
            }
          ...
          for (let j = decryptPool[0].length - 1, i; j > -1; j--) {
            ...
            for (i = 0; i < decryptPool.length; i++) {
              ...
              if (char) {
                ...
                stringKeyIndex =
                  (stringKey - (stringKeyIndex % stringKey)) % stringKey;
            ...
            if (keys) {
              for (let i = 0, j; i < strings.length; i++) {
                for (j = 0; j < keys.length; j++) {
                  strings[i] =
                    i % 2 === 0
                      ? keys[j].encrypt(strings[i])
                      : keys[j].decrypt(strings[i]);
                }
              }
              ...
              // 为了使加密方法不一样，这里反过来
              strings[i] = this[randomkey].decrypt(
                this[randomkey].decrypt(strings[i])
              );
              return strings.length === 1 ? strings[0] : strings;
            }
          }
        ...
        if (arguments) {
          ...
          if (strings) {
            if (keys) {
              ...
              strings[i] = this[randomkey].encrypt(
                this[randomkey].encrypt(strings[i])
              );
              return strings.length === 1 ? strings[0] : strings;
            }
          }
  ...
  (() => {
    if (
      location.search.match(/[?&]url=https?:\/\/.+\..+\..+\/.+/i) ||
    ...
    if (
      ...
      if (window === top) {
        ...
        for (const name in settings.parseDB) {
          const data = localStorage.getItem("解析数据库." + name);
          if (data !== null) {
        ...
        const toolsBar = document.createElement("toolsbar");
        toolsBar.style =
        ...
        toolsBar.innerHTML =
          ...
          "    </ul>\n" +
          "</settings>";
        const SettingsBlock = toolsBar.querySelector("settings>button+ul");
        const parseDBKeys = Object.keys(settings.parseDB);
        for (let i = 0; i < parseDBKeys.length; i++) {
          SettingsBlock.innerHTML +=
            "<li><text style='left: 10px'>" +
            parseDBKeys[i] +
            "</text><label class='parse-switch' style='right: 10px;'><input type='checkbox'><bg></bg><span></span></label></li>\n";
        }
        const SettingBlockSwitchs = SettingsBlock.querySelectorAll(
          "li>label.parse-switch"
        );
        for (let i = 0; i < SettingBlockSwitchs.length; i++) {
          const checkBox = SettingBlockSwitchs[i].querySelector(
            "input[type=checkbox]"
          );
          checkBox.checked = Boolean(settings.parseDB[parseDBKeys[i]]);
          SettingBlockSwitchs[i].querySelector("bg").addEventListener(
            "transitionend",
            () => {
              if (
                checkBox.checked !== Boolean(settings.parseDB[parseDBKeys[i]])
        ...
        const SettingsBtn = toolsBar.querySelector("settings>button");
        SettingsBtn.onclick = () => {
  ...
  function start() {
          ...
          () =>
            doElement(
              ...
              document.readyState !== "complete"
                ? waitMS
                : waitMS - 10 - settings.getElementTimes
            ),
    ...
    function doElements(cssString, doFunction, waitMS = 0, index = 0) {
      const Elements = document.querySelectorAll(cssString);
      if (Elements[index] && Elements[index].nodeType === 1) {
        doFunction(Elements);
        console.log(
          "%c已为 All[" + index + "] " + cssString + " 进行了操作",
          settings.fontStyle.ok
        );
      } else if (document.readyState !== "complete" || waitMS > 0) {
        ...
        console.log(
          "%c未找到 All[" + index + "] " + cssString,
          settings.fontStyle.error
        );
      }
    }
    ...
    function forElements(
      ...
      const forElementInterval = setInterval(() => {
        if (document.readyState !== "complete" || waitMS > 0) {
          ...
          if (Elements && Elements.length > 0 && Elements[0].nodeType === 1) {
            ...
            console.log(
              "%cforElements已为 " + cssString + " 进行了操作",
              settings.fontStyle.ok
            );
          }
    ...
    function removeElements(ElementsStrings) {
      ...
      const removeElementsInterval = setInterval(() => {
        if (ElementsStrings.length > 0) {
          for (const i in ElementsStrings) {
            try {
              const Elements = eval(ElementsStrings[i]);
              if (Elements && Elements.nodeType === 1) {
                console.log(
                  "%cremoveElemets 执行了移除 " + ElementsStrings[i],
                  settings.fontStyle.ok
                );
          ...
          if (document.readyState === "complete") {
            console.log(
              "%cremoveElemets 移除失败 " + ElementsStrings,
              settings.fontStyle.error
            );
    ...
    if (window === top) {
              ...
              if (sql[1] === "函数") {
                ...
                if (sql[2] === "用户数据库") {
                  event.source.postMessage(
                    key.ENCRYPT(
                      "天王盖地虎\x00给予\x00用户数据库\x00" +
                        JSON.stringify(settings.parseDB),
                      settings.k1,
                      settings.k2
                    ),
                    "*"
      ...
      if (!detectMobile()) {
        if (location.host.indexOf("v.qq.com") !== -1) {
          readyPlayerBox(
            ...
            [
              ...
              ".mod_vip_popup",
              "#mask_layer",
              "div.panel-overlay.panel-tip-pay",
              "div.thumbplayer-barrage",
            ],
            [
              settings.NoAD解析["OK解析"],
              settings.NoAD解析["TV解析"],
              settings.NoAD解析["天翼解析"],
              settings.NoAD解析["诺讯解析"],
            ],
            "div#player",
            null
          );
        } else if (location.host.indexOf("iqiyi.com") !== -1) {
          doElement("div.side-cont.tvg", () =>
            readyPlayerBox(
              "已进入爱奇艺",
              ["#playerPopup", "div[class^=qy-header-login-pop]"],
              [settings.NoAD解析["OK解析"]],
              "iqpdiv.iqp-player[data-player-hook$=er]",
              null
            )
          );
          ...
          doElements(
            "div[role=tooltip]:not([class*=popover-])",
            (loginTip) => displayNone(["#" + loginTip[6].id]),
            1000,
            6
          );
          doElement("svg[aria-hidden=true]", () =>
            readyPlayerBox(
              "已进入哔哩哔哩",
              null,
              [settings.NoAD解析["夜幕解析"], settings.NoAD解析["天翼解析"]],
              "div.bpx-player-video-area,div.mask-container",
              null
            )
          ...
          readyPlayerBox(
            "已进入搜狐视频",
            null,
            [settings.NoAD解析["夜幕解析"]],
            "#player,#sohuplayer,.player-view",
            null
          );
          ...
          readyPlayerBox(
            ...
            null,
            [settings.NoAD解析["夜幕解析"]],
            "div.teleplayPage__playerSection",
            null
          );
        ...
        if (location.host.indexOf("v.qq.com") !== -1) {
          readyPlayerBox(
            ...
            [settings.NoAD解析["OK解析"]],
            "div.mod_play:not([style*='display: none;']) section.mod_player",
            null,
            (href) => {
          ...
          readyPlayerBox(
            ...
            [
              "#iframaWrapper",
              ".ad-banner-wrapper",
              ".h5-detail-guide,.h5-detail-vip-guide,[class$=ad],.Corner-container",
              "[data-spm='downloadApp'],.downloadApp",
              ".callEnd_box",
            ],
            [],
            "#player",
          ...
          readyPlayerBox(
            "已进入哔哩哔哩",
            [
              "div.fe-ui-open-app-btn,div.recom-wrapper,open-app-btn",
              "[class*=openapp]",
            ],
            [settings.NoAD解析["夜幕解析"], settings.NoAD解析["天翼解析"]],
            "div#app.main-container div.player-wrapper",
            null,
            (href) => href.replace("m.bilibili.com", "www.bilibili.com")
          );
        } else if (location.host.indexOf("miguvideo.com") !== -1) {
          readyPlayerBox(
            "已进入咪咕视频",
            [
              "[class^=app_],[class^=app-],[class*=_app_],[class*=-app-],[class$=_app],[class$=-app]",
              ".openClient",
              "div.group-item.programgroup .data-rate-01,div.group-item.programgroup .max-rate-01,div.group-item.programgroup .p-common",
            ],
            [],
            "section#mod-player",
            null,
            (href) =>
              href
                .replace("m.miguvideo.com", "www.miguvideo.com")
                .replace("msite", "website")
          );
        } else if (location.host.indexOf("le.com") !== -1) {
          ((block_show) => {
            block_show.innerHTML =
              "div.layout{visibility: visible !important; display:block !important;}div.layout>*:not(style,script,#j-vote,#j-follow){visibility: visible !important; display: block !important;}";
          ...
          readyPlayerBox(
            ...
            [
              "a.leapp_btn",
              "div.full_gdt_bits[id^=full][data-url]",
              "[class*=Daoliu],[class*=daoliu],[class*=game]",
              "div.m-start",
              "[class*=icon_user]",
            ],
            [settings.NoAD解析["云解析"]],
          ...
          readyPlayerBox(
            ...
            [
              ...
              "[class$=-app]",
              "div.app-vbox.ph-vbox,div.app-vbox.app-guess-vbox",
              "div.twinfo_iconwrap",
            ...
            async (href) => {
              return new Promise((resolve) => {
                xmlHttpRequest({
                  url: href,
                  success: (data) => {
                    const result = data.responseText.match(
                      /var videoData = \{[^\x00]+tvUrl:"(http.+)",[\r\n]/
                    )[1];
                    resolve(result);
                  },
                  error: () => resolve(href),
          ...
          readyPlayerBox(
            ...
            [
              ...
              "div[class^=mg-app],div#comment-id.video-comment div.ft,div.bd.clearfix,div.v-follower-info",
              "div.ht.mgui-btn.mgui-btn-nowelt",
              "div.personal",
              "div[data-v-41c9a64e]",
            ],
            [
            ...
            "div.video-poster,div.video-area",
            null
          );
        } else if (location.host.indexOf("ixigua.com") !== -1) {
          readyPlayerBox(
            ...
            [
              "div.xigua-download",
              "div.xigua-guide-button",
              "div.c-long-video-recommend.c-long-video-recommend-unfold",
            ],
            [settings.NoAD解析["夜幕解析"]],
            "div.xigua-detailvideo-video",
            null
          );
        } else if (location.host.indexOf("pptv.com") !== -1) {
          readyPlayerBox(
            ...
            [
              ...
              "div[class^=pp-m-diversion]",
              "section#ppmob-detail-picswiper",
              "section.layout.layout_ads",
              "div.foot_app",
              "div[modulename=导流位]",
              "a[class*=user]",
              "div.mod_video_info div.video_func",
            ],
            [settings.NoAD解析["OK解析"]],
            "section.pp-details-video",
            null,
            (href) => href.replace("m.pptv.com", "v.pptv.com")
          );
        } else if (location.host.indexOf("vip.1905.com") !== -1) {
          ((movie_info) => {
            movie_info.innerHTML =
              "section#movie_info{padding-top: 20px !important;}";
            document.head.appendChild(movie_info);
          })(document.createElement("style"));
          readyPlayerBox(
            "已进入1905电影网",
            [
              "a.new_downLoad[target=_blank]",
              "iframe[srcdoc^='<img src=']",
              "section.movieList_new.club_new",
              ".wakeAppBtn",
              "[class*=login]",
              "section.openMembershipBtn",
              ".ad",
              ".open-app,.openApp,ul.iconList li:not(.introduceWrap),div#zhichiBtnBox",
              "section#hot_movie,section#exclusive_movie,section#hot_telve",
            ],
            [settings.NoAD解析["云解析"]],
            "div.area.areaShow.clearfix_smile",
            null
          );
        } else if (location.host.indexOf("www.wasu.cn") !== -1) {
          readyPlayerBox(
            ...
            [
              "div.ws_poster",
              "div.appdown,div.player_menu_con",
              "div#play_and_info_fix_adv",
            ],
            [settings.NoAD解析["OK解析"]],
            "div#player,div#pop",
            null
      ...
      function readyPlayerBox(
        ...
        doElement(cssString, async (playerBox) => {
          ...
          settings.randomSeleceParse = async () => {
            ...
            if (srcs.length > 0) {
              const random = Math.floor(Math.random() * srcs.length);
          ...
          settings.DIY_iframeFunction = () => {
            if (settings.parseDB.DIY解析栏) {
              if (
                !DIY_iframe ||
                DIY_iframe.constructor.name !== "HTMLDivElement"
              ) {
                // 自定义解析地址
                DIY_iframe = document.createElement("div");
                DIY_iframe.style =
                  "display: flex; background-color: #0000; font-family: 微软雅黑,黑体,Droid Serif,Arial,sans-serif; font-size: 15px;" +
                ...
                const DIY_iframe_text =
                  DIY_iframe.querySelector("input[type=text]");
                ...
                async function DIY_iframe_src() {
                  ...
                  settings.src.set(
                    DIY_iframe_select.options[DIY_iframe_select.selectedIndex]
                      .value,
                    DIY_iframe_text.value
                      ? DIY_iframe_text.value
                      : location.href
                  );
                ...
                const DIY_iframe_select = DIY_iframe.querySelector("select");
                for (const name in settings.NoAD解析) {
                  DIY_iframe_select.innerHTML +=
                    "<option value='" +
                    settings.NoAD解析[name] +
                    "' style='text-align: center'>" +
                    name +
                    "</option>";
                }
                for (const name in settings.AD解析) {
                  DIY_iframe_select.innerHTML +=
                    "<option value='" +
                    settings.AD解析[name] +
                    "' style='text-align: center; color: #fa0; '>⚠" +
                    name +
                    "</option>";
                }
                DIY_iframe.querySelector(
                  "option[value='" + settings.src.get() + "']"
                ).selected = true;
                settings.randomSeleceParse = async () => {
                  // arguments 代表输入的所有参数，看不懂可以百度搜索 “js 参数 arguments”
                  // return arguments ? arguments[Math.floor(Math.random() * arguments.length)] : null;
                ...
                DIY_iframe.querySelector("button").onclick = () =>
                  DIY_iframe_src();
          ...
          ((waiter) => {
            ...
            const resetPlayerBoxInterval = setInterval(() => {
              const newPlayerBox = document.querySelector(cssString);
              if (
                (newPlayerBox !== playerBox && newPlayerBox !== null) ||
                (newPlayerBox !== null &&
                  newPlayerBox.querySelector(
          ...
          setInterval(() => {
            ...
            for (const other_iframe of document.querySelectorAll(
              "iframe,fake-iframe-video"
            )) {
              if (
                other_iframe.src !== iframe.src ||
                other_iframe.constructor.name !== iframe.constructor.name
              ) {
      ...
      function setParseVideo() {
        forElements(
          ...
          async (videos, thisInterval) => {
            for (const video of videos) {
              ...
              if (video.src) {
                ...
                top.postMessage(
                  key.ENCRYPT(
                    "宝塔镇河妖\x00函数\x00settings.address = null",
                    settings.k1,
                    settings.k2
                  ),
                  "*"
                ...
                top.postMessage(
                  key.ENCRYPT(
                    "宝塔镇河妖\x00函数\x00localStorage.setItem('historyParse', settings.src.get())",
                    settings.k1,
                    settings.k2
                  ),
                  "*"
                ...
                if (settings.parseDB.弹幕开关 === "\x00") {
                  ...
                  removeElements([
                    ...
                    'document.querySelector("div[class$=player-danmu]")',
                ...
                if (!detectMobile()) {
                  const fullscreen_btn = document.querySelector(
                    "[class*=fullscreen],[class$=player-full] button[class$=full-icon]"
                  );
                  if (fullscreen_btn && fullscreen_btn.nodeType === 1) {
                    // 阻止事件冒泡
                    fullscreen_btn.parentNode.parentNode.onkeyup = (event) =>
                      event.stopPropagation();
                  }
                  video.onkeyup = (event) => event.stopPropagation();
                  top.postMessage(
                    key.ENCRYPT(
                      "宝塔镇河妖\x00按下Enter获取焦点",
                      settings.k1,
                      settings.k2
                    ),
                    "*"
          ...
          function () {
            top.postMessage(
              key.ENCRYPT(
                "宝塔镇河妖\x00给予\x00-1\x00" + location.href,
                settings.k1,
                settings.k2
              ),
              "*"
      ...
      if (location.host.indexOf("jiexi.t7g.cn") !== -1) {
        ...
        ((style) => {
          style.innerHTML =
            ".slide,.panel,.slide *,.panel *{width: 0 !important; max-width: 0 !important; opacity: 0 !important;}";
        ...
        if (location.pathname.indexOf("jx.php") === -1) {
          ...
          // 移除背景图片
          doElement(
            "div[style*='width:100%;height:100%;'][style*='.jpg']",
            (background) => {
              background.style =
                "width:100%;height:100%;position:relative;z-index:2147483647987;";
            },
            5000
          );
        }
    ...
    function onLocationChange(handler) {
      ...
      const onLocationChangeInterval = setInterval(() => {
        console.log("正在监听url");
        const href = top.location.pathname;
  ...
  function showTip(msg, style = "") {
    ...
    if (window === top) {
      ...
      tip.style =
        style +
        "pointer-events: none; opacity: 0; background-color: #222a; color: #fff; font-family: 微软雅黑,黑体,Droid Serif,Arial,sans-serif; font-size: 20px; text-align: center; padding: 6px; border-radius: 16px; position: fixed; transform: translate(-50%, -50%); left: 50%; bottom: 15%; z-index: 2147483647;";
      tip.innerHTML =
        "<style>@keyframes showTip {0%{opacity: 0;} 33.34%{opacity: 1;} 66.67%{opacity: 1;} 100%{opacity: 0;}}</style>\n" +
        msg;
      const time = msg.replace(/\s/, "").length / 2; // TODO 2个字/秒
      // cubic-bezier(起始点, 起始点偏移量, 结束点偏移量, 结束点)，这里的 cubic-bezier函数 表示动画速度的变化规律
      tip.style.animation =
        "showTip " +
        (time > 2 ? time : 2) +
        "s cubic-bezier(0," +
        (time - 1 > 0 ? (time - 1) / time : 0) +
        "," +
        (1 - (time - 1 > 0 ? (time - 1) / time : 0)) +
        ",1) 1 normal";
